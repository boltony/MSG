...
<div class=col-9 style="margin:auto; text-align:center; width:900px;min-width:900px;
">
<h1>일괄전송</h1>
<br>
...
<tr></tr>
<tr class="tt1"><td name="seq">${dto.seq}</td><td name="writer">${dto.writer}</td>
<td name="title" style="max-width:350px;word-break:break-all">${dto.title}</td>
<td name="YN">${dto.accept }</td>
<td style="max-width:188px">

	<input type="button" value="내용보기" id="show${dto.seq}"/></td></tr>
<tr class="hide" id="cont${dto.seq }" style="display:none" style="height:300px;max-width:188px; background-color : #f6fff2;">
<td colspan=5 style="padding-left:10px;text-align:left;text-indent:10px;max-width:497px;word-break:break-all">내용 : ${dto.contents}</td></tr>
<tr class="accept_btn" id="con${dto.seq}" style="display:none;text-align:right">
<td colspan=5 style="max-width:497px;"><input type="button" value="내용숨기기" id="hide${dto.seq}"/>
<input type="button" value="일괄전송" id="accept${dto.seq}">
<input type="button" value="삭제" id="del${dto.seq}">
</td></tr>

   <script>
  $("#del${dto.seq}").on("click",function(){
     location.href = "${pageContext.request.contextPath}/del.sos?seq=${dto.seq}"
  })
$('#show${dto.seq}').on("click",function(){
  ...
  $('#hide${dto.seq}').on("click",function(){
	  $("#show${dto.seq}").show();
     $('#con${dto.seq}').hide();
      $('#cont${dto.seq}').css('display','none');
      
   })
   $('#accept${dto.seq}').on("click",function(){   //<!-- 일괄전송 누를때 contents(내용)가지고 서블릿으로감  -->
      
         location.href="${pageContext.request.contextPath}/acceptY.mem?contents=${dto.contents}"
   })
